...
class ReplayMemory:
    ...
    def add_transition(self, transitions_new):
        if self.size == 0:
            # Initialize with the correct shape and empty objects
            n_elements = len(np.asarray(transitions_new, dtype=object))
            self.transitions = np.empty((self.max_size, n_elements), dtype=object)
...
class PrioritizedExperienceReplay(ReplayMemory):
    def __init__(
        self,
        max_size=100000,
        beta_1=0.6,        # PER α parameter (used in priority calculation)
        beta_2=0.4,        # starting β value (your per_beta)
        beta_frames=100000,  # total frames over which β should anneal to 1.0  # CHANGED
        epsilon=1e-6
    ):
        ...
        self.beta_1 = beta_1
        self.beta_2 = beta_2  # starting value for β
        self.beta_frames = beta_frames  # CHANGED: use beta_frames for annealing horizon
        self.epsilon = epsilon
        self.max_priority = 1.0

        # A frame counter for the annealing schedule
        self.frame = 1
    ...
    def sample(self, batch=1, step=None, total_steps=None):
        ...
        # Compute current β using the frame-based schedule:
        current_beta = min(1.0, self.beta_2 + self.frame * (1.0 - self.beta_2) / self.beta_frames)  # CHANGED
        self.frame += 1

        # Compute importance-sampling weights with current β
        p_min = self.min_tree.min() / total_p
        max_weight = (p_min * self.size) ** (-current_beta)
        for i, idx in enumerate(indices):
            p_sample = self.sum_tree[idx] / total_p
            weight = (p_sample * self.size) ** (-current_beta)
    ...
    def get_state(self):
        ...
        per_state = {
            ...
            "max_priority": self.max_priority,
            "beta_frames": self.beta_frames,  # CHANGED: store beta_frames (not self.frame)
        }
    ...
    def set_state(self, state):
        ...
        self.beta_frames = state.get("beta_frames", self.beta_frames)  # CHANGED: restore beta_frames if available
...
class EREPrioritizedExperienceReplay(PrioritizedExperienceReplay):
    ...
    def __init__(
        ...
        eta_T=1.0,
        c_k_min=2500,
        beta_frames=100000  # CHANGED: use beta_frames instead of beta_increment
    ):
        # CHANGED: Pass beta_frames to the parent and remove beta_increment.
        super().__init__(max_size, beta_1, beta_2, beta_frames, epsilon)
        # ERE parameters
        self.eta_0 = eta_0
        self.eta_T = eta_T
    ...
    def sample(self, batch=1, step=None, total_steps=None):
        ...
        # CHANGED: Use frame-based schedule for β here as well:
        current_beta = min(1.0, self.beta_2 + self.frame * (1.0 - self.beta_2) / self.beta_frames)
        self.frame += 1
        
        max_weight = (p_min * len(valid_indices)) ** (-current_beta)
        for i, idx in enumerate(indices):
            p_sample = self.sum_tree[idx] / total_p
            weight = (p_sample * len(valid_indices)) ** (-current_beta)
            weights[i] = weight / max_weight
        # Removed the old fixed beta update line.
        transitions = self.transitions[indices]
        states = np.array([t[0] for t in transitions])
        actions = np.array([t[1] for t in transitions])
        rewards = np.array([t[2] for t in transitions])
